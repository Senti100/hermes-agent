...
| `kanban_block` | Stop work and route by why: `kind=dependency` waits in `todo` and auto-resumes **only when an unfinished parent exists**; parentless/all-terminal dependency blocks become sticky `needs_input` blocks. `needs_input`/`capability`/`transient` surface to a human. Repeated same-kind re-blocks auto-escalate to `triage`. | `reason` |
...
| `dependency_wait` | `{reason, kind}` | Worker blocked with `kind=dependency` while at least one parent is unfinished, so it routes to `todo` (parent-gated, auto-promoted) instead of `blocked`. Parentless or all-terminal dependency requests become sticky `needs_input` blocks to prevent immediate `todo → ready` retry storms. |
